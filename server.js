...
app.get('/api/partidos', async (req, res) => {
  try {
    res.json(await db.getPartidos(req.query.ronda));
...
app.post('/api/registro', async (req, res) => {
  ...
  try {
    if (await db.existeEmail(emailLower))
...
app.post('/api/prediccion', async (req, res) => {
  ...
  try {
    let part = participante_id
      ? await db.getParticipanteById(participante_id)
      : await db.getParticipante(email.toLowerCase().trim());
...
app.get('/api/pdf/quiniela/:id', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="quiniela_${nombre}.pdf"`);
    doc.pipe(res);
    doc.fontSize(20).text('⚽ Quiniela Mundial 2026 — Segunda Fase', { align: 'center' });
    doc.fontSize(12).text('Creado por Mario Vitale', { align: 'center' }).moveDown();
    if (!part) {
    ...
    doc.fontSize(11).text(`⭐ Balón de Oro: ${award?.prediccion||part.balon_oro||'—'}`).moveDown();
    const rondas  = ['R32','R16','QF','SF','3P','F'];
    const nombres = { R32:'Ronda de 32', R16:'Octavos', QF:'Cuartos', SF:'Semis', '3P':'Tercer Lugar', F:'Gran Final' };
