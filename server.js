...
const db = {
  ...
  async getPartidos(ronda) {
    if (MODO_LOCAL) {
      return ronda
        ? DB.partidos.filter(p => p.ronda === ronda)
        : [...DB.partidos];
    }
  ...
  async upsertPrediccion(data) {
    if (MODO_LOCAL) {
      ...
      if (idx !== -1) {
        DB.predicciones[idx] = { ...DB.predicciones[idx], ...data };
      } else {
        DB.predicciones.push({ ...data, id: 'PR' + Date.now() });
      }
      return;
    }
...
app.get('/api/partidos', async (req, res) => {
  try {
    ...
    const partidos  = await db.getPartidos(ronda);
    res.json(partidos);
  } catch(e) {
...
app.put('/api/admin/participante/:id', authAdmin, async (req, res) => {
  ...
  try {
    ...
    await db.updateParticipante(id, updates);
    res.json({ ok: true });
  } catch(e) {
    res.status(500).json({ ok: false, error: e.message });
  }
